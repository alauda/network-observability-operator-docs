...
2. In the left navigation, go to **Observe** > **Network Traffic**.
...
Use the **Time Range** control on the right side of the query bar.
...
When the selected field supports a direction, choose **Source**, **Destination**, or **Both** before you apply the value.
...
If packet drop tracking is enabled, the **Drops** control can limit the table to:
...
Use **Manage Columns** to show additional columns such as Project, owner, node, zone, protocol, bytes, packets, DNS, RTT, TLS, or packet drop fields.
