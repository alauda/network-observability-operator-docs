...
2. The Network Observability Operator
...
If you already have a ClickHouse server for NetObserv, skip this section and go to [Install the Network Observability Operator](#install-the-netobserv-operator).
...
3. If the ClickHouse Operator status is _Absent_, upload the ClickHouse Operator package that you downloaded from the **<Term name="company"/> Customer Portal**.
...
## Install the Network Observability Operator \{#install-the-netobserv-operator}
...
7. Enables the Console Network Traffic plugin.
8. Namespace where the eBPF agents and the flow logs pipeline are deployed.
9. Number of replicas for the flow logs pipeline.
...
The example explicitly enables the console plugin with `spec.consolePlugin.enable: true`. This field defaults to `true` when omitted.
After the FlowCollector instance is ready, open the cluster console and go to **Observe** > **Network Traffic** to inspect Overview, Flows, and Topology.
